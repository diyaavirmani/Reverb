...
import { DecisionRejectedAlternativeSchema } from "../../../../schemas";
...
const requestSchema = z
  .object({
    ...
    selectedReasons: z.array(z.string().min(1)).min(1),
    rejectedAlternatives: z.array(DecisionRejectedAlternativeSchema),
    riskFacts: z.array(z.string().min(1)).optional()
  })
  .strict();
...
export async function POST(request: Request) {
  ...
  try {
    const result = await createOpenAIAdapter().explainProviderDecision(parsed.data);
